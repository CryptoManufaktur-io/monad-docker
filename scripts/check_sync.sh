...
Examples:
  ...
  ./scripts/check_sync.sh --public-rpc https://monad-rpc.publicnode.com:443
  ./scripts/check_sync.sh --compose-service monad --public-rpc https://monad-rpc.publicnode.com:443
  CONTAINER=monad-1 PUBLIC_RPC=https://monad-rpc.publicnode.com:443 ./scripts/check_sync.sh
USAGE
}
...
rpc_call() {
  ...
  if [[ -n "$CONTAINER" ]]; then
    docker exec "$CONTAINER" sh -c "curl -sS --fail --max-time 10 -X POST -H 'Content-Type: application/json' --data '$data' '$url' 2>/dev/null"
  else
    curl -sS --fail --max-time 10 -X POST -H 'Content-Type: application/json' --data "$data" "$url" 2>/dev/null
  fi
}
...
if ! local_result="$(rpc_call "$LOCAL_RPC" "eth_blockNumber" "[]")"; then
  fail_sync "RPC unreachable (${LOCAL_RPC})"
fi

if ! public_result="$(rpc_call "$PUBLIC_RPC" "eth_blockNumber" "[]")"; then
...
if ! local_height_hex="$(printf '%s' "$local_result" | jq_eval '.result // empty')"; then
  fail_sync "JSON parse failure (local RPC)"
fi
if ! public_height_hex="$(printf '%s' "$public_result" | jq_eval '.result // empty')"; then
  fail_sync "JSON parse failure (public RPC)"
fi

if [[ -z "$local_height_hex" || -z "$public_height_hex" ]]; then
  fail_sync "missing block number in RPC response"
fi
...
local_syncing="false"
if syncing_result="$(rpc_call "$LOCAL_RPC" "eth_syncing" "[]")"; then
  local_syncing_raw="$(printf '%s' "$syncing_result" | jq_eval '.result // empty')"
  if [[ "$local_syncing_raw" != "false" && -n "$local_syncing_raw" ]]; then
    local_syncing="true"
  fi
fi

sync_state="unknown"
if [[ "$local_syncing" == "true" ]]; then
  sync_state="syncing"
...
echo "Local latest:  ${local_height_dec} (${local_height_hex})"
echo "Public latest: ${public_height_dec} (${public_height_hex})"
